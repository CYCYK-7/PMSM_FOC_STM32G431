...
volatile uint32_t iu_offset_sum = 0;
volatile uint32_t iv_offset_sum = 0;

volatile uint16_t iu_offset_adc = 2048;
volatile uint16_t iv_offset_adc = 2048;

volatile int32_t iu_adc_delta = 0;
volatile int32_t iv_adc_delta = 0;

volatile int32_t iu_ma = 0;
volatile int32_t iv_ma = 0;


volatile uint32_t current_offset_count = 0;
volatile uint32_t adc_injected_count = 0; // the number of times ADC synchronous sampling occurs

volatile uint8_t current_offset_done = 0;
...
DQ_t i_dq = {0};
volatile float theta_e_test = 0.0f; //test angle
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim6); //Start TIM6 and allow it to work in interrupt mode.
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_1);// Truly activate the PWM output of TIM4 Channel 1. INIT only be used to configure TIM4
...
void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    {
        iu_raw_sync =
            HAL_ADCEx_InjectedGetValue(&hadc1,
                                       ADC_INJECTED_RANK_1);

        current_sample_ready |= 0x01; //01
    }

    if (hadc->Instance == ADC2)
    {
        iv_raw_sync =
            HAL_ADCEx_InjectedGetValue(&hadc2,
                                       ADC_INJECTED_RANK_1);

        current_sample_ready |= 0x02; //10
    }

    /* U、V两相本周期数据都已经到齐 */
    if (current_sample_ready == 0x03) // 11
    {
        ...
        if (foc_state == FOC_STATE_CALIBRATION)
        {
            ...
            {
                iu_offset_adc =
                    (uint16_t)(iu_offset_sum / CURRENT_OFFSET_SAMPLES);
